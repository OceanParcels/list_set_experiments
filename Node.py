...
class Node(object):
    ...
    def __init__(self, prev=None, next=None, id=None, data=None):
        ...
        if id is not None and isinstance(id, int) and (id>=0):
            ...
            self.id = package_globals.idgen.nextID()
        else:
            self.id = None
        self.data = data
...
class NodeJIT(Node, ctypes.Structure):
    ...
    def __init__(self, prev=None, next=None, id=None, data=None):
        ...
        #    node_c_interface = code_compiler.InterfaceC("node")
        #    node_c_interface.compile_library()
        #    node_c_interface.load_library()
        #node_c_interface.register()
        ...
        #ctypes.cast(self, ctypes.c_void_p)
        #ctypes.addressof()
        #getattr(self, '_c_next_p').ctypes.data_as()
        #getattr(self, index).ctypes.data_as(c_void_p)
    ...
    def __del__(self):
        ...
        #    node_c_interface.unregister()
        #    if node_c_interface.register_count <= 0:
        #        node_c_interface.unload_library()
        #        node_c_interface = None

    def unlink(self):
        # print("NodeJIT.unlink() [id={}] is called.".format(self.id))
        if self.registered:
            # print("Calling ctype funcs.")
            if self.prev is not None:
                if self.next is not None:
                    self.prev.set_next(self.next)
                else:
